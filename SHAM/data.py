...
class ProcessData:
    ...
    def get_processed_data(self, data, ids, date, ordinal_col, nominal_col, groupby,
        ...
        if date is None:
            data.insert(1, 'Date', pd.date_range(end='30/01/2023',
                                                 periods=len(data)))
    ...
    def split_dataset(self,
        ...
        if regression:
            ...
            trainX, testX_df_ids, \
            trainY, testY_df = train_test_split(data.iloc[:, :-1],
                                                data.iloc[:, -1],
                                                test_size=0.2,
            ...
            testY_df_ids = pd.DataFrame(testY_df).copy()
            testY_df_ids.insert(0, 'ID', np.array(testX_df_ids['ID']))

            trainX_df_ids, valX_df_ids, \
            trainY_df, valY_df = train_test_split(trainX,
                                                  trainY,
                                                  test_size=0.2,
                                                  stratify=trainY)
            ...
            trainY_df_ids = pd.DataFrame(trainY_df).copy()
            trainY_df_ids.insert(0, 'ID', np.array(trainX_df_ids['ID']))

            valY_df_ids = pd.DataFrame(valY_df).copy()
            valY_df_ids.insert(0, 'ID', np.array(valX_df_ids['ID']))

            train_df = pd.concat([trainX_df_ids, trainY_df], axis=1)
